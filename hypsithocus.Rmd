...
raw_data.df <- read_excel("hypsithocus_raw.xlsx", sheet = "combined_data")
```

```{r}
# Summarise sample size and average eggs per mass for each pentatomid species x treatment

pents_sum <- raw_data.df %>% 
...
# Plot average percent egg parasitism and total mass acceptance
...
ggplot(emerge_plot, aes(y = reorder(species, proportion), proportion, fill = metric)) +
  ...
  scale_fill_manual(values = c("#D1C7B5", "#6C90B9"), 
                    name = "", breaks = c("acceptance", "avg_wasps"), 
                    labels = c("Total Mass Acceptance", "Average Egg Parasitism"))  +
...
para_spread <- raw_data.df %>% 
  filter(treatment %in% c("no-choice")) %>% 
  mutate(
    emergence = (n_wasps_m + n_wasps_f) / n_eggs,
  ) 

ggplot(para_spread, aes(emergence, species)) +
  geom_violin() +
  geom_point()
```
...
str(raw_data.df)
raw_data.df$source <- as.factor(raw_data.df$source)
raw_data.df$species <- as.factor(raw_data.df$species)
raw_data.df$treatment <- as.factor(raw_data.df$treatment)

# Filter out control obs

no_ctrl.df <- raw_data.df %>% 
...
hyp_means <- emmeans(model_1, ~ species) 

# Get post-hoc pairwise comparisons to see where the differences lie

hyp_con <- contrast(hyp_means, method = "pairwise", adjust = "fdr")
write.table(hyp_con, file = "hyp-conmeans.txt", sep = ",", quote = FALSE, row.names = F)
